...
public class MyGraphics {
	...
	public void elevateMultiple(Graphics g, int num) {
		for(int i = 0; i<num; i++) {
			thePoly.elevateOnce();
		}
		Color color = g.getColor();
	...
	public void updatePoint(Graphics g, int updateIdx, int newX, int newY) {
		thePoly.xpoints[updateIdx] = newX;
		thePoly.ypoints[updateIdx] = newY; 
		Color color = g.getColor();
		g.setColor(paintColor);
		drawPolyPoints(g);
		g.drawPolyline(thePoly.xpoints,thePoly.ypoints,thePoly.npoints);
		if(thePoly.elevated) {
			int numElevations = thePoly.numElevations;
			thePoly.elevated = false;
